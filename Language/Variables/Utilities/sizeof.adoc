...
이 프로그램은 한번에 한 글자씩 텍스스 문자열을 출력한다. 텍스트 구문을 바꾸는 것을 시도해 보시오.
...
[float]
=== 주의와 경고
`sizeof` 는 전체 바이트 수를 반환하는 것을 주의하시오. int 와 같은 형의 큰 변수는, for 루프는 이렇게 보일 수 있다. 또 적절한 형식의 문자열이 NULL 로 끝나는데, 그것은 아스키 값 0을 갖는 것을 주의하세요.
