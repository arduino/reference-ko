...
[float]
=== 설명
C 언어에서 변수는, 아두이노가 사용하는데, scope 라 불리는 속성을 가진다. 이것은 모든 변수가 _전역_ 인 BASIC 같은 언어의 옛 버전에서와 대조적이다.
...
때때로 변수를 `for` 루프 안의 변수를 선언하고 초기화하는게 편하다. 이것은 for 루프 중괄호 안에서만 접근 가능한 변수를 만든다.
