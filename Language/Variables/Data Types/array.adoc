...
이런 까닭에 배열에 액세스할 때 조심해야 한다.배열 끝을 지나서 액세스하면(선언한 배열크기 - 1 보다 큰 인덱스 번호 사용) 다른 목적으로 사용중인 메모리를 읽는다. 이 위치에서부터 읽는 것은 무효 데이터를 얻는 것 말고는 아무일을 안 할 겁니다. 임의 메모리 위치에 쓰는 것은 분명 나쁜 생각이며, 충돌 또는 프로그램 오작동 같은 불행한 결과가 될 수 있다. 찾기 어려운 버그가 될 수 있다.
[%hardbreaks]
베이식이나 자바와 달리, C 컴파일러는 배열 액세스가 선언한 배열 크기 법적 범위 안에 있는지 체크하지 않는다.
[%hardbreaks]
